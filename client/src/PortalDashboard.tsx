...
function useResource(api: Api, path: string) {
...
export function PortalDashboard({ session, onLogout }: { session: Session; onLogout: () => void }) {
  const [view, setView] = useState<(typeof navigation)[number][0]>(() => (location.hash.slice(1) as any) || "dashboard");
  ...
  const visibleNavigation = navigation.filter(([id]) => id !== "admin" || isAdmin);
  const page = {
    ...
    clinical: <ClinicalPage api={api} canManageCatalog={isOwner || isAdmin} />,
    staff: <StaffPage api={api} editable={isOwner || isAdmin} />,
    documents: <DocumentsPage api={api} token={session.token} />,
    appointments: <AppointmentsPage api={api} isAdmin={isAdmin} />,
    messages: <MessagesPage api={api} />,
    contracts: <ContractsPage api={api} isAdmin={isAdmin} />,
    invoices: <InvoicesPage api={api} token={session.token} canCreate={isOwner || isAdmin} />,
    payments: <PaymentsPage api={api} token={session.token} isAdmin={isAdmin} />,
    reports: <ReportsPage api={api} isAdmin={isAdmin} />,
    claims: <ClaimsPage api={api} token={session.token} isAdmin={isAdmin} />,
...
function ClinicalPage({ api, canManageCatalog }: { api: Api; canManageCatalog: boolean }) {
...
function MessagesPage({ api }: { api: Api }) {
  ...
  return <><section className="panel-card"><h2>Contact billing support</h2><form className="stack-form" onSubmit={create}><input name="subject" placeholder="Subject" required /><textarea name="body" placeholder="Message" required /><button className="primary">Start conversation</button></form>{error && <div className="error">{error}</div>}</section><PageState busy={resource.busy} error={resource.error} /><div className="card-list">{resource.rows.map((row) => <article className="resource-card" key={row._id}><span className="status-badge">{row.status}</span><h3>{row.subject}</h3><div className="message-history">{row.messages?.map((message: Row) => <p key={message._id}><strong>{message.sender?.fullName ?? "User"}:</strong> {message.body}</p>)}</div><button onClick={() => void reply(row)}>Reply</button></article>)}</div></>;
}

function ContractsPage({ api, isAdmin }: { api: Api; isAdmin: boolean }) {
  const resource = useResource(api, "/contracts"); const [error, setError] = useState("");
  async function create(event: FormEvent<HTMLFormElement>) { event.preventDefault(); try { await api("/contracts", { method: "POST", body: JSON.stringify(formValues(event)) }); event.currentTarget.reset(); await resource.load(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Create failed"); } }
...
function ReportsPage({ api, isAdmin }: { api: Api; isAdmin: boolean }) {
  ...
  async function create(event: FormEvent<HTMLFormElement>) { event.preventDefault(); try { await api("/reports", { method: "POST", body: JSON.stringify(formValues(event)) }); event.currentTarget.reset(); await resource.load(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Report failed"); } }
...
function ClaimsPage({ api, token, isAdmin }: { api: Api; token: string; isAdmin: boolean }) {
  const resource = useResource(api, "/claims"); const [error, setError] = useState("");
  async function create(event: FormEvent<HTMLFormElement>) { event.preventDefault(); try { await api("/claims", { method: "POST", body: JSON.stringify(formValues(event)) }); event.currentTarget.reset(); await resource.load(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Claim failed"); } }
...
function displayValue(key: string, value: any) {
  ...
  if (value && typeof value === "object") return value.fullName ?? value.invoiceNo ?? value.name ?? "-";
  return String(value ?? "-");
}
